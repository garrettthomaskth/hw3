function [Q,x,t,cons] = mccormack(xSteps,ratio,Tend,alpha,epsilon)
...
for i = 1:tSteps+1
    ...
    for j = 2:xSteps+1
        ustar = Q(j,2*i) - dt/dx*(f(Q(j+1,(2*i-1):(2*i)))-f(Q(j,(2*i-1):(2*i))));
        Q(j,2*i+1:2*i+2) = 1/2*(Q(j,(2*i-1):(2*i))-ustar - dt/dx*(f(Q(j+1,(2*i-1):(2*i)))-f(Q(j-1,(2*i-1):(2*i)))));
    end
end
